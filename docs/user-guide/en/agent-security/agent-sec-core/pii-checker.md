...
| `regex` | Yes | One regex expression; V2 accepts only the `m` and `s` inline flags |
...
patterns; examples include branch-reset groups, named conditionals, variable-length lookbehind,
and inline comment groups `(?#...)`.
Patterns that compile with different meanings are also rejected: the `$` assertion, `\h`, `\H`,
`\Z`, word-edge escapes `\<` / `\>`, nested character classes and unescaped class operators
`&&`, `--`, `~~`, `||`. V2 accepts only the `m` / `s` inline flags; `i` and `x` are rejected
because Unicode case folding and verbose parsing differ. Escaped literals such as `\$`,
`[$]`, and `\\h` remain valid. Rejection disables the whole custom collection and reports
`invalid_regex` with partial coverage; built-ins continue. Validate migrated rules on both matching
and non-matching inputs, including final newlines and Unicode. Administrators must explicitly revise
rejected rules; V2 never translates them automatically.
