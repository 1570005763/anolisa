...
| `regex` | 是 | 单个正则表达式；V2 仅接受 `m` 和 `s` 内联 flag |
...
例如 branch-reset 分组、命名条件分支、变长 lookbehind 及内联注释分组 `(?#...)`。
支持数字条件分支和定长 lookaround。
对能够编译但含义不同的模式也会拒绝：`$` 断言、`\h`、`\H`、`\Z`、词边界转义
`\<` / `\>`、嵌套字符类及字符类中未转义的 `&&`、`--`、`~~`、`||` 运算符。
V2 仅接受 `m` / `s` 内联 flag；`i` 和 `x` 因 Unicode 大小写折叠及扩展模式解析差异而被拒绝。
`\$`、`[$]` 和 `\\h` 等转义字面量仍然有效。拒绝时整份自定义集合无效，报告
`invalid_regex` 和 partial coverage，内置检测继续。迁移规则需要验证正反例，包含末尾换行
及 Unicode 输入；管理员必须明确修改被拒绝的规则，V2 不会自动转换。
