...
| 工作人员：下载离线包 | [本版全部文件](https://github.com/1570005763/anolisa/releases/tag/agentseccore-demo-20260911.1) |
...
工作人员在窗口 A 确认本次演示目录后输入 `/login`。使用 Qoder CLI 原生登录；ECS 优先选择 **Personal Access Token**，按原生提示粘贴从活动账号获取的 PAT。本机也可以选择浏览器登录。账号认证及首次 SSH 主机确认由工作人员完成，详见 [Qoder CLI 官方认证说明](https://docs.qoder.com/cli/authentication)。
...
这会结束 A 的 Qoder CLI、恢复原始 Skill 并扫描到 `pass`，保留账号、签名密钥和历史事件。随后**回到 A**重新进入：本机运行 `./demo.sh qoder`，ECS 在本机重新运行 `bash ecs-demo.sh user@ecs-host`。B 继续保留原控制终端，不在 B 启动 Qoder CLI。
...
- **文件校验失败**：停止使用被修改的安装文件，排查原因。安装器只接受当前版或已知未修改的 `20260910.1`，更新时保留私密配置、离线镜像和数据卷，成功后清理临时文件。
