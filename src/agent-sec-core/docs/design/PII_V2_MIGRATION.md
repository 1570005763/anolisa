...
64 nested groups. Unsupported Python syntax and known semantic differences are rejected as
`invalid_regex` without rewriting: `$`, `\h`/`\H`/`\Z`, word-edge escapes, nested/set-algebra
character classes, and inline flags other than `m`/`s`. Escaped literals remain supported.
Administrators validate positive and negative cases, including final newlines and Unicode, before
activating central rules. A rejected rule invalidates the whole custom collection with partial coverage.
