...
引擎深度计数可能拒绝 64 层嵌套分组。不支持的 Python 语法及已知语义差异均以
`invalid_regex` 拒绝，不改写规则，包括 `$`、`\h`/`\H`/`\Z`、词边界转义、嵌套/集合运算
字符类及除 `m`/`s` 外的内联 flag；转义字面量仍受支持。管理员在启用集中规则前需验证正反例，
包含末尾换行和 Unicode。任一规则被拒绝时整份自定义集合无效，并标记 partial coverage.
