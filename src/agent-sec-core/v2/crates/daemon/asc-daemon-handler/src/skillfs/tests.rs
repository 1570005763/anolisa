...
fn resolver_aliases_share_identity_and_reject_false_mappings_without_fallback() {
    ...
    let bridge =
        SkillFsBridge::start(fixture.config(), service.clone(), Finalizer::new(events)).unwrap();
...
fn resolver_accepts_reserved_leaf_inside_a_category_only() {
    ...
    let bridge = SkillFsBridge::start(
        fixture.config(),
        fixture.service(),
        Finalizer::new(Arc::new(Events::default())),
    )
    .unwrap();
...
fn rotation_commands_resolve_current_inodes_and_retain_failed_intents() {
    for command in [
        ...
        let events = Arc::new(Events::default());
        let service = fixture.service();
        let finalizer = Finalizer::new(events);
        let bridge = Arc::new(
            SkillFsBridge::start(fixture.config(), service.clone(), finalizer.clone()).unwrap(),
        );
        ...
        let mut handler = crate::skill_sec::SkillSecHandler::new(service.clone(), finalizer);
        handler.skillfs = Some(bridge.clone());
        let invoke = || {
            serde_json::to_value(handler.handle(
...
async fn public_socket_authenticates_notify_and_keeps_v2_requests_separate() {
    ...
    let dispatcher = Arc::new(
        ...
        .with_skill_sec(service.clone(), finalizer)
        .with_skillfs(bridge.clone()),
    );
    ...
    let server = UnixService::new(
        ...
        transport_config(),
        dispatcher,
        Arc::new(crate::JsonRejectionEncoder),
    )
    .unwrap();
