...
use asc_capability_skill_sec::{SkillRoot, SkillSecError, SkillSecService};
...
impl SkillSecHandler {
    ...
    pub(super) fn handle(
        ...
        let prepared = match &command {
            SkillSecCommand::RotateKeys {} => self.service.rotation_skills(control.deadline()),
...
mod tests {
    ...
    fn consumer_errors_and_kernel_identity_remain_distinct_from_protocol_errors() {
        ...
        let handler = SkillSecHandler::new(service, Finalizer::new(events.clone()));
    ...
    fn skill_budget_does_not_change_ordinary_methods_or_embedding_defaults() {
        ...
        .with_skill_sec(service, Finalizer::new(Arc::new(Events::default())));
        for payload in [
