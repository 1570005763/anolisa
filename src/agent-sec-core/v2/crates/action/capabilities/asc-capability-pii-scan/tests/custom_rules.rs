...
#[test]
fn python_semantic_differences_invalidate_the_whole_custom_collection() {
    for pattern in [
        r"^INTERNAL-[0-9]+$",
        r"INTERNAL\h+[0-9]+",
        r"INTERNAL\H+[0-9]+",
...
fn supported_syntax_and_escaped_literals_keep_their_match_boundaries() {
    // Match boundaries are also checked against Python regex VERSION0 during
    // migration acceptance; unsupported syntax is never silently rewritten.
...
fn file_pattern_rule_and_nesting_bounds_are_enforced() {
    ...
    // The 64-group configuration ceiling does not override the engine's own
    // recursion accounting, which includes its root parse frame.
