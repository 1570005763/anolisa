...
fn compile(document: &str) -> Result<Vec<CompiledRule>, RuleError> {
    ...
    for item in items {
        ...
        validate_pattern_shape(expression)?;
...
fn validate_pattern_shape(pattern: &str) -> Result<(), RuleError> {
    let mut depth: usize = 0;
    let mut escaped = false;
    let mut in_class = false;
    let mut class_can_close = false;
    let mut class_start = 0;
    for (index, c) in pattern.char_indices() {
        if escaped {
            // These escapes compile in both engines but have different meanings.
            // Do not silently reinterpret an administrator's Python rule.
            if matches!(c, 'h' | 'H' | 'Z') || (!in_class && matches!(c, '<' | '>')) {
                return Err(RuleError::InvalidRegex);
            }
            escaped = false;
            class_can_close |= in_class;
        } else if c == '\\' {
            escaped = true;
        } else if in_class {
            // Python regex defaults to VERSION0 simple sets; Rust accepts nested
            // sets and set algebra with different membership for the same text.
            if c == '['
            ...
            if c == ']' && class_can_close {
                in_class = false;
            } else if c != '^' || index != class_start + 1 {
            ...
            // Python also accepts the position before a final LF. The target
            // engine's default dollar assertion means the absolute end only.
            return Err(RuleError::InvalidRegex);
        } else if c == '(' {
            depth += 1;
            // fancy-regex does not preserve Python's named-condition branches.
            // Literal text in escapes/classes must not disable the collection.
            let named_condition = pattern[index..].starts_with("(?(")
                && !pattern[index + 3..].starts_with(|c: char| c.is_ascii_digit());
            // Inline comments can contain arbitrary regex punctuation; keep them
            // outside this lexical subset rather than misreading subsequent syntax.
            if depth > MAX_DEPTH || named_condition || pattern[index..].starts_with("(?#") {
                return Err(RuleError::InvalidRegex);
            }
            if let Some(flags) = pattern[index..].strip_prefix("(?") {
                // Only m/s flag groups belong to the supported common subset.
                // In particular, Unicode case folding and verbose lexing differ.
                if flags.starts_with(|c: char| c.is_ascii_alphabetic() || c == '-')
                    && !flags.starts_with("P<")
                    && !flags.starts_with("P=")
                {
                    let flags = flags.split([':', ')']).next().unwrap_or(flags);
                    if flags.chars().any(|c| !matches!(c, 'm' | 's' | '-')) {
                        return Err(RuleError::InvalidRegex);
                    }
                }
            }
...
mod tests {
    ...
    #[test]
    fn conditional_guard_does_not_reject_literal_character_classes() {
