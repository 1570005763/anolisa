...
use asc_capability_skill_sec::command::SkillSecCommand;
use asc_capability_skill_sec::executor::{
    SkillSecAuditProjector, SkillSecExecutor, SkillSecRequest,
};
use asc_capability_skill_sec::scanner::{ScannerConfig, ScannerRegistry};
use asc_capability_skill_sec::{
    SkillIdentity, SkillRoot, SkillSecConfig, SkillSecError, SkillSecService,
};
...
pub(super) fn recover(
    ...
    let runtime = ActionRuntime::new(
        ActionId::SkillSec,
        SkillSecExecutor::new(service.clone()),
    ...
    if service.key_status(deadline)?["rotationPending"] == true {
        let roots = service
            ...
            .collect();
        let outcome = run(SkillSecCommand::RotateKeys {}, roots);
        if !outcome.success {
    ...
    if service.key_status(deadline)?["initialized"] == true {
        for identity in service.managed_skills()? {
            let root = match bridge.map_or_else(
                ...
                Err(error) => {
                    eprintln!(
                        "agent-sec-daemon: SkillSec startup mapping unavailable for {}: {error}",
            ...
            if !outcome.success {
                eprintln!(
                    "agent-sec-daemon: SkillSec reconcile failed for {}: {}",
                    identity.path().display(),
                    outcome.error_type
                );
            }
