...
| HTML page rendering | workload-dependent | Opt in with `TOKENLESS_HTML_EXTRACTION_ENABLED=1` or SDK `html_extraction_enabled=True`; renders complete HTML documents from commands or APIs as Markdown, removes only enumerated non-content elements (scripts, styles, navigation, banners, footers, asides) with counts in the view header, and provides original recovery. Disabled by default; file reads pass through |
