...
| HTML 页面转写 | 取决于工作负载 | 通过 `TOKENLESS_HTML_EXTRACTION_ENABLED=1` 或 SDK `html_extraction_enabled=True` 启用；把命令或 API 返回的完整 HTML 文档转写为 Markdown，只移除可枚举的非内容元素（脚本、样式、导航、页眉、页脚、侧栏）并在视图头部计数，提供原文恢复。默认关闭；文件读取透传 |
